...
author: "Abdullahi" # Remember to add your name!
theme: united
---
...
pacman::p_load(plotly, tidyverse, reactable, here, scales,
               htmltools, crosstalk, plotly, dplyr, shiny)
```

```{r}
## Import data
cantril <-read_csv(here("data/happiness-cantril-ladder.csv")) %>%
...
cantril_country <- cantril %>%
  filter(Country ==c("Ghana", "Kenya", "Nigeria", "Senegal","Rwanda"))

cantril_plots <- ggplot(data=cantril_country,
       aes(x=Year,
  ...
  labs(
    x="Year",
    y= "Ladder score",
    title= "World happiness score over time for selected countries in Africa"
  )
  ggplotly(cantril_plots)
...
#our data manipulation and plots
shared_cantril_data <- highlight_key(cantril)
...
top_10 <- head(cantril_2023, 10)
bottom_10 <- tail(cantril_2023, 10)

top_bottom_10 <- bind_rows(top_10, bottom_10) %>%
  mutate(rank_group = ifelse(row_number() <= 10, "Top 10", "Bottom 10"))

top_bottom_plot <- plot_ly(
    data = top_bottom_10,
    x = ~`Cantril ladder score`,
    y = ~reorder(Country, `Cantril ladder score`),
    type = "bar",
    orientation = "h",
    color = ~rank_group,
    ...
    layout(
        ...
        xaxis = list(title = "Cantril Ladder Score"),
...
browsable(
    tagList(
        tags$h2("World Happiness Report Explorer"),
        navlistPanel(
            "Navigation",
            tabPanel("Top & Bottom 10 in 2023", top_bottom_plot)
        )
    )
)
